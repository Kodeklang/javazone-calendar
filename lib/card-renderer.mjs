...
export async function withCardRenderer(chars, body) {
  ...
  try {
    ...
    //
    // palette: false is spelled out rather than left to libvips. Quantising to
    // 256 colours costs no visible quality and saves a few kB a card, but it
    // puts a quantiser between the hex values in the source and the pixels,
    // and on artwork whose whole job is to look like the site that is not a
    // trade worth the kilobytes.
    //
