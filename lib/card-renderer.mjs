...
export async function withCardRenderer(chars, body) {
  ...
  try {
    ...
    // spends its palette on first. Measured on three cards, a sparse one, a
    // busy one and one in between: every pixel of open ground comes back
    // bit-identical, max delta 0 over 151527 sampled pixels, and the 1-2% of
    // pixels that do move are the antialiasing along type and wordmark edges,
    // at most 29 of 255 and invisible at 3x magnification.
    //
    ...
    //
    // Off by default all the same, because src/icons/og.png is committed art
    // that predates this and must stay byte-identical.
