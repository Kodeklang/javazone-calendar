...
// here is a knob that moves the bytes, and it is one object rather than
// arguments scattered over the call because scripts/build-session-cards.mjs
// hashes it into each card's manifest entry: raster settings decide the output
// just as much as the SVG does, so changing any of them has to redraw the set.
// A setting left outside this object would change the files while the manifest
...
// like: it is both better *and* smaller than every lossy setting worth having,
// so there is nothing here to trade off. Measured on the same three session
// cards plus the site-wide one, encoding what `render` below actually hands
// the encoder: lossless lands at 75-81% of the PNG and is bit-identical to it,
// where lossy at quality 100 costs 175-258% of the PNG and still moves pixels
// by up to 96 of 255 - ringing along the edges of type, which is nearly all
// this image is. Only quality 80 undercuts lossless on size, at 88-115%, and
// it rings just as hard. nearLossless is not a middle ground on this input at
// any quality: it produced identical bytes to plain lossless every time, the
// card having only 256 colours in it to begin with.
//
// effort is libwebp's search budget and 6 is its maximum. 0 is catastrophic
// here - two to eleven times the size - and 2 and 4 land within 2% of 6, so
// this is the cheap end of a flat curve rather than a fight for the last byte.
//
// `quality` under `lossless` is not a fidelity knob at all: it selects how
// hard the lossless coder looks for redundancy, and every value is bit-exact.
// 100 is worth another 4% and costs about four seconds a card against fifty
// milliseconds at the default. That is paid only when a card is actually
// redrawn - a design change, or a session whose title moved - and never on the
// hourly run over an unchanged programme, which writes nothing at all.
export const RASTER = {
  removeAlpha: true,
...
export async function withCardRenderer(chars, body) {
  ...
  try {
    ...
    /**
     * A finished card, in both formats a crawler is offered.
     *
     * The SVG is rasterised once, into the PNG, and the WebP is then encoded
     * from that PNG's own pixels rather than from a second pass over the SVG.
     * Two rasterisations would agree - librsvg is deterministic, which this
     * whole file exists to keep true - but the two files are published as
     * alternates of one image and an unfurler picks whichever it likes, so it
     * is worth making them the same image rather than merely equal ones. The
     * quantisation the PNG applies is inside that guarantee: it is what a
     * reader taking the PNG sees, so it is what a reader taking the WebP
     * should see too. Decoding a PNG is cheap next to setting type through
     * librsvg, so this costs less than drawing the card twice would.
     *
     ...
     * the pixels are identical. That also rules out restoring metadata around
     * a raw-pixel round trip, which was the other way to encode both from one
     * raster - sharp will only put the density back by attaching an ICC
     * profile with it.
     */
    const render = async (svg, raster = RASTER) => {
      const drawn = sharp(Buffer.from(svg));
      if (raster.removeAlpha) drawn.removeAlpha();
      const png = await drawn.png(raster.png).toBuffer();
      return { png, webp: await sharp(png).webp(raster.webp).toBuffer() };
    };
